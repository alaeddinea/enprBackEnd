...
public class AdministrationController {
	...
	@GetMapping("/message")
	public ApiResponse<List<Administration>> message() {
		 return new ApiResponse<>(HttpStatus.OK.value(), " admin message bien recu .",
				null);
	}
	@GetMapping("/all")
	public ApiResponse<List<Administration>> listConge() {
		return new ApiResponse<>(HttpStatus.OK.value(), "Conge  list fetched successfully.",
				administrationRepository.findAll());
	}
	@GetMapping("/getone/{id}")
	public ApiResponse<Administration> getCongeById(@PathVariable("id") long id) {
		Optional<Administration> congeData = administrationRepository.findById(id);

		if (congeData.isPresent()) {
			return new ApiResponse<>(HttpStatus.OK.value(), "Conge  fetched successfully", administrationRepository.findById(id));
		} else {
	...
	@PostMapping("/add")
	public ApiResponse<Administration> save(@RequestBody Administration administration) {
		try {
	...
	@PutMapping("/update")
	public ApiResponse<Administration> update(@RequestBody  Administration administration) {
		try {
