package com.enpr.controller;

import java.util.List;
import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.enpr.dao.RepoEleveRepository;
import com.enpr.dao.TestpsyEleveRepository;
import com.enpr.model.ApiResponse;
import com.enpr.model.RepoEleve;
import com.enpr.model.TestpsyEleve;


@CrossOrigin(origins = "*", maxAge = 3600)
@RestController
@RequestMapping("/api/testpsyeleve")
public class TestpsyEleveController {
	@Autowired
	private TestpsyEleveRepository testpsyEleveRepository   ;

	@GetMapping("/all")
	public ApiResponse<List<RepoEleve>> listConge() {
		return new ApiResponse<>(HttpStatus.OK.value(), "Conge  list fetched successfully.",
				testpsyEleveRepository.findAll());
	}
	@GetMapping("/getone/{id}")
	public ApiResponse<TestpsyEleve> getCongeById(@PathVariable("id") long id) {
		Optional<TestpsyEleve> congeData = testpsyEleveRepository.findById(id);

		if (congeData.isPresent()) {
			return new ApiResponse<>(HttpStatus.OK.value(), "Conge  fetched successfully", testpsyEleveRepository.findById(id));
		} else {
			return new ApiResponse<>(HttpStatus.NOT_FOUND.value(), "Conge  Not Found.", null);
		}

	}

	@PostMapping("/add")
	public ApiResponse<TestpsyEleve> save(@RequestBody TestpsyEleve testpsyEleve) {
		try {

			return new ApiResponse<>(HttpStatus.OK.value(), "Conge saved successfully.",
					testpsyEleveRepository.save(testpsyEleve));
		} catch (Exception e) {
			return new ApiResponse<>(HttpStatus.EXPECTATION_FAILED.value(), "Conge not Saved", null);
		}

	}
	@PutMapping("/update")
	public ApiResponse<TestpsyEleve> update(@RequestBody  TestpsyEleve testpsyEleve) {
		try {

			return new ApiResponse<>(HttpStatus.OK.value(), "Conge updated successfully.",
					testpsyEleveRepository.save(testpsyEleve));
		} catch (Exception e) {
			return new ApiResponse<>(HttpStatus.NOT_FOUND.value(), "Conge not Saved", null);
		}

	}
	@DeleteMapping("/delete/{id}")
	public ApiResponse<Void> delete(@PathVariable("id") long id) {
		try {
			testpsyEleveRepository.deleteById(id);
			return new ApiResponse<>(HttpStatus.NO_CONTENT.value(), "Conge  Deleted", null);
		} catch (Exception e) {
			return new ApiResponse<>(HttpStatus.EXPECTATION_FAILED.value(), "Conge not Deleted", null);
		}
	}
	@GetMapping("/findAllSonByMother/{id}")
	public ApiResponse<List<RepoEleve>> findAllSonByMother(@PathVariable("id") long id) {
		
		List<TestpsyEleve> testpsyEleves = testpsyEleveRepository.findAllSonByMother(id);
		 
		if (!testpsyEleves.isEmpty() ) {
			return new ApiResponse<>(HttpStatus.OK.value(), "groups fetched suucessfully", testpsyEleves);
		}
		else
		{
			return new ApiResponse<>(HttpStatus.NOT_FOUND.value(), "groups Not FOund", null);
		}
	}
	
}


