...
@RestController
@RequestMapping("/api/medicale")
public class MedicaleController {
	@Autowired
	private MedicaleRepository medicaleRepository  ;

	@GetMapping("/all")
	public ApiResponse<List<Medicale>> listConge() {
		return new ApiResponse<>(HttpStatus.OK.value(), "Conge  list fetched successfully.",
				medicaleRepository.findAll());
	}
	@GetMapping("/getone/{id}")
	public ApiResponse<Medicale> getCongeById(@PathVariable("id") long id) {
		Optional<Medicale> medicaleData = medicaleRepository.findById(id);

		if (medicaleData.isPresent()) {
			return new ApiResponse<>(HttpStatus.OK.value(), "Conge  fetched successfully", medicaleRepository.findById(id));
		} else {
	...
	@PostMapping("/add")
	public ApiResponse<Medicale> save(@RequestBody Medicale medicale) {
		try {

			return new ApiResponse<>(HttpStatus.OK.value(), "Conge saved successfully.",
					medicaleRepository.save(medicale));
		} catch (Exception e) {
	...
	@PutMapping("/update")
	public ApiResponse<Medicale> update(@RequestBody  Medicale medicale) {
		try {
	...
	public ApiResponse<Void> delete(@PathVariable("id") long id) {
		try {
			medicaleRepository.deleteById(id);
